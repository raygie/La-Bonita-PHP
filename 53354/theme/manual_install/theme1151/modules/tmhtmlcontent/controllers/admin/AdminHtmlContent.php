...
class AdminHtmlContentController extends ModuleAdminController
{
	public function ajaxProcessUpdatePosition()
	{
		...
		$success = true;
		for ($i = 1; $i <= $total; $i++)
			$success &= Db::getInstance()->update('tmhtmlcontent', array('item_order' => $i), '`id_item` = '.preg_replace('/(item-)([0-9]+)/', '${2}', $items[$i - 1]));
		if (!$success)
