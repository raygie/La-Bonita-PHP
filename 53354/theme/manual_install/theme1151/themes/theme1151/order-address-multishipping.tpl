...
<div class="addresses clearfix">
	...
	{else}
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn-sm btn btn-default icon-right">
        	<span>
	...
	<p class="address_add submit">
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back|urlencode}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn-sm btn btn-default icon-right">
        	<span>
            	{l s='Add a new address'}
            </span>
        </a>
...
{if !$opc}
	<p class="cart_navigation clearfix">
		...
		{if $back}
			<a href="{$link->getPageLink('order', true, NULL, "back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn btn-default icon-left">
                <span>{l s='Continue Shopping'}</span>
            </a>
		{else}
				<a href="{$link->getPageLink('order', true, NULL)|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn btn-default icon-left">
                <span>{l s='Continue Shopping'}</span>
            </a>
		{/if}
...
{strip}
    ...
    {capture}
        <a class="btn-sm btn btn-default icon-right" href="{$smarty.capture.addressUrlAdd}" title="{l s='Update' js=1}">
            <span>
                {l s='Update' js=1}
            </span>
        </a>
